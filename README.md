...
A money or rule field is `{ value, sourceUrl, lastVerified, confidence, sourceNote }`. `lastVerified` moves to today only when that park’s parser is confident. The row says **Rate updated September 21, 2026** (the real date) when that read succeeded, **Call the park for the latest rate** when there is no confirmed rate, and **Rate unconfirmed** when the old number is still on the row. A page that will not open does not invent a rate. The check log still records `stale` or `blocked`.
...
A page that will not open, or a page the parser cannot read, keeps the old number and the row says **Rate unconfirmed**. The job does not invent a rate.
